    ...
    _Scrolleo.prototype = {
        init: function() {
            ...
            Array.prototype.forEach.call(this.wrapper, function(wr) {
                ...
                var setVideoToBlob = function() {
                            // console.log("Downloading video...")
                        var xhr = new XMLHttpRequest();
                        xhr.open('GET', self.wrapper[0].src, true);
                        xhr.responseType = 'blob';
                        xhr.onload = function(e) {
                            if (this.status == 200) {
                                    // console.log("Video downloaded...");
                                self.myBlob = this.response;
                                self.vid = (window.URL ? URL : URL).createObjectURL(self.myBlob);
                                // myBlob is now the blob that the object URL pointed to.
                                self.video = document.getElementById(self.wrapper[0].id);
                                    // console.log("Loading video into element" + self.wrapper[0]);
                                self.wrapper[0].src = self.vid;
                    ...
                    getElemDistanceToTop = function(elem) {
